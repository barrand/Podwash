...
#
# Primary CLI: scripts/forge.sh
# Primary UI:  scripts/forge-floor.sh → Start Forge
#
# Bootstraps the Cursor SDK venv, then runs the Medic supervisor → forge_loop.py.
# All arguments are passed through (supervisor strips its own flags).
#
# Usage:
#   scripts/forge.sh                 # drain queue (Medic on by default)
#   scripts/forge.sh --dry-run       # show the next decision; spawns no agent
#   scripts/forge.sh --max 3         # run at most 3 items this session
#   scripts/forge.sh --no-self-heal  # plain forge_loop — no Medic on halt
#   scripts/forge.sh --medic-no-push # Medic commits heal locally, skips push
#
# Auth (non-dry-run): export CURSOR_API_KEY=cursor_...
#
# Requires Python 3.10+ for cursor-sdk (macOS /usr/bin/python3 is often 3.9).
# Override: PODWASH_PYTHON=python3.12 scripts/forge.sh
#
...
export PODWASH_FORGE_LOOP="${PODWASH_FORGE_LOOP:-forge_loop}"
...
find_python310() {
    if [ -n "${PODWASH_PYTHON:-}" ]; then
        ...
        echo "forge.sh: PODWASH_PYTHON=$PODWASH_PYTHON is not Python 3.10+" >&2
        return 1
    fi
    ...
    echo "forge.sh: cursor-sdk requires Python 3.10+." >&2
...
if ! venv_ok; then
    if [ -d "$VENV" ]; then
        echo "forge.sh: recreating venv ($PYTHON required; old venv unusable)"
        rm -rf "$VENV"
    else
        echo "forge.sh: creating venv at $VENV ($PYTHON)"
    fi
    "$PYTHON" -m venv "$VENV"
